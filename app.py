...
def main():
    ...
    st.set_page_config(page_title="Hellsing Attack", layout="wide")
    ...
    current_language = getattr(Config, "LANGUAGE", None)
    if current_language is None:
        Config.load_config()
        current_language = getattr(Config, "LANGUAGE", "english")
    
    # Set index to show current language in selectbox
    ...
    if language != current_language:
        ...
        Config.save_config()
        # Reload translator to pick up new language
        t.reload()
        st.rerun()
    ...
    if attack_type == t.get("port_scan"):
        ...
        if st.button(t.get("port_scan")):
            if target:
                with st.spinner(
                    t.get("scanning_ports").format(start_port, end_port, target)
                ):
        ...
        num_connections = st.number_input(
            t.get("num_connections_prompt"), 1, 10000, 100
        )
